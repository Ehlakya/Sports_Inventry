...
const AdminProducts = () => {
  ...
  const closeModal = () => { setModal(null); setSelected(null); setForm(EMPTY_FORM); setImageFile(null); };
            ...
            <form onSubmit={handleSave} className="overflow-y-auto flex-1 p-6 space-y-5">
              ...
              <div>
                ...
                <div className="flex items-center gap-4 mt-2">
                  <div className="h-16 w-16 rounded-xl border border-slate-200 dark:border-slate-700 flex items-center justify-center bg-slate-50 dark:bg-slate-800/50 overflow-hidden flex-shrink-0 shadow-sm">
                    {imageFile ? (
                      <img src={URL.createObjectURL(imageFile)} alt="preview" className="h-full w-full object-cover" />
                    ) : form.imageUrl && !imgPreviewError ? (
                      <img src={form.imageUrl} alt="preview" onError={() => setImgPreviewError(true)} className="h-full w-full object-cover" />
                    ) : (
                      <ImageIcon className="h-6 w-6 text-slate-400" />
                    )}
                  </div>
                  <div className="flex-1">
                    <label className="cursor-pointer inline-flex items-center gap-2 px-4 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-sm font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors shadow-sm">
                      <Plus className="h-4 w-4" /> Choose Image
                      <input 
                        type="file" 
                      ...
                      {form.sizes.map((row, i) => (
                        <div key={i} className="flex gap-2 items-center">
                          <input
                            ...
                            placeholder={isPredefined ? row.size : "Enter Size"}
                            className={`w-28 px-3 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-sm font-semibold text-center text-slate-800 dark:text-white placeholder-slate-400 dark:placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-900/40 ${isPredefined ? 'bg-slate-50 dark:bg-slate-800/50 cursor-not-allowed' : ''}`}
                          />
                          <input
                          ...
                          {!isPredefined && (
                            <button
                              ...
                              className="p-2.5 rounded-xl text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 disabled:opacity-30 transition-colors"
                            >
